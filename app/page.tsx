...
const q=questions[i];
useEffect(()=>{if(q.skill==="Listening"&&typeof window!=="undefined"){window.speechSynthesis.cancel();const u=new SpeechSynthesisUtterance(q.q);u.lang="en-US";u.rate=0.82;u.pitch=1;window.speechSynthesis.speak(u)}return()=>{if(typeof window!=="undefined")window.speechSynthesis.cancel()}},[i,q.q,q.skill]);
const playListening=()=>{if(typeof window==="undefined"||q.skill!=="Listening")return;window.speechSynthesis.cancel();const u=new SpeechSynthesisUtterance(q.q);u.lang="en-US";u.rate=0.82;u.pitch=1;window.speechSynthesis.speak(u)};
const score=useMemo(()=>answers.reduce((s,a,n)=>s+(a&&correct(questions[n],a)?1:0),0),[answers]);
